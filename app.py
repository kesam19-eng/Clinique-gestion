...
# --- 1. CONFIGURATION ---
st.set_page_config(page_title="DONKA MANAGER", page_icon="🏥", layout="wide", initial_sidebar_state="collapsed")
...
# INITIALISATION (AVEC LE PATIENT M. DIALLO PAR DÉFAUT)
if 'patients' not in st.session_state:
    ...
    st.session_state.patients = demo_data
...
if menu == "📊 VUE GLOBALE":
    ...
    with g1: 
        ...
        if not df_pat.empty: st.plotly_chart(px.pie(df_pat, names='Acte', hole=0.4), use_container_width=True)
    with g2:
        st.subheader("État Clinique")
        if not df_pat.empty: st.dataframe(df_pat[['Nom', 'Diagnostic', 'Statut']], use_container_width=True)
    ...
    with t2:
        ...
        else: st.warning("Aucun patient dans la base.")

    with t3:
        if not df_pat.empty:
            # On reprend le sélecteur pour être sûr
            ...
            with c_cam:
                ...
                img = st.camera_input("Photo Radio") if src == "Caméra" else st.file_uploader("Upload")
